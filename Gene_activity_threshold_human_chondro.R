...
gene_FPKM <- read.csv('project_results/humanchondro_bulk/RSEM_tables/RSEM_FPKM_genes.csv', row.names = 1, header = TRUE)

# Load gene annotation info
gene_annotations <- read.csv('data/humanchondro_bulk/Gene_annotation.csv', row.names = 1, header = TRUE)

# Calculate activity threshold for each sample in each dataset
# --------------------------------------------------------------------------

# Make list of expression dataframes
FPKM_list <- list(gene_FPKM)
...
gene_annotations$humanchondro_all <- gene_annotations$humanchondro_activity == 5
gene_annotations$humanchondro_any <- gene_annotations$humanchondro_activity > 0

# Write gene annotation csv with gene activity information
write.csv(gene_annotations, "project_results/humanchondro_bulk/Gene_annotation_w_activity.csv")
