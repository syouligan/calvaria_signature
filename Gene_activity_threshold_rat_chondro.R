...
gene_FPKM <- read.csv('project_results/ratchondro_bulk/RSEM_tables/RSEM_FPKM_genes.csv', row.names = 1, header = TRUE)
rownames(gene_FPKM) <- sub("(.*?)\\..*", "\\1", rownames(gene_FPKM))

# Load gene annotation info
gene_annotations <- read.csv('data/ratchondro_bulk/Gene_annotation.csv', row.names = 1, header = TRUE)
...
gene_annotations$ratchondro_mean_FPKM <- rowMeans(gene_annotations[,grepl("wk_mean_FPKM", colnames(gene_annotations))])

# Write gene annotation csv with gene activity information
write.csv(gene_annotations, "project_results/ratchondro_bulk/Gene_annotation_w_activity.csv")
